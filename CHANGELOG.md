...
### Fixed
- **Langfuse / RAGFlow** - `make update` failed at the image pull with `pull access denied for minio/minio, repository does not exist`: MinIO removed its Docker Hub repositories in September 2026 (the community edition is now source-only). Both `minio` and `ragflow-minio` now pull `quay.io/minio/minio`, which still serves the last published image, `RELEASE.2025-09-07`; it is a frozen snapshot, so MinIO will not receive further updates from upstream. Existing data volumes are untouched (#119).
